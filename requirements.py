            ...
            for i in range(num_charts):
                ...
                with col1:
                    ...
                    if x_column and y_columns:
                        ...
                        with col_start:
                            x_start = st.number_input(f"Enter X-axis start for Chart {i+1}", min_value=x_min, max_value=x_max, value=x_start, key=f"x_start_{i}")
                        with col_end:
                            x_end = st.number_input(f"Enter X-axis end for Chart {i+1}", min_value=x_min, max_value=x_max, value=x_end, key=f"x_end_{i}")
